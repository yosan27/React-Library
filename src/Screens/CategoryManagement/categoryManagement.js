...
export default class CategoryManagement extends Component {
    ...
    findCategory() {
        axios.get("http://localhost:8500/api/category")
            .then((response) => {
    ...
    addCategory = () => {
        ...
        if (this.state.button === "Add Category") {
            ...
            axios
                .put(`http://localhost:8500/api/update-category/${this.state.id}`, categoryList)
                .then(() => window.location.reload());
        }
    };
